...
findCorrelation_hi=function(cor_matrix,cor_pvalue_matrix,threshold){
  ...
  order_increase=order(apply(x1, 1, function(x){mean(x,na.rm=T)}))
  ...
  for (i in colnames(cor_matrix)) {
    ...
    temp_cor_index=temp_cor_index[apply(temp_cor_index, 1, function(x){length(unique(x))==2}),]
    ...
    if(i %in% colnames(x1)){
      if(any(x1[,i]>=threshold,na.rm = T)){
        ...
        continue=TRUE
        rep=0
        col_name=colnames(x1)[which(colnames(x1)==i)]
        rm_metric="ignore"
        ...
        while (continue) {
          ...
          if(x2[j,i]<=0.05)
          {
            ...
            name1=colnames(x1)[which(colnames(x1)==i)]
            name2=colnames(x1)[j]
            ...
            if(col_name==rm_metric){continue=FALSE}

            x1=x1[-c(which(colnames(x1)==rm_metric)),-c(which(colnames(x1)==rm_metric))]
            x2=x2[-c(which(colnames(x2)==rm_metric)),-c(which(colnames(x2)==rm_metric))]
