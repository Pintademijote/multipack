...
Cor_test=function(Varia_paysage_multi,method_cor){
  ...
  # colnames(Varia_paysage_multi)=paste0(colnames(Varia_paysage_multi),"_")

  pb <- txtProgressBar(min = 0, max = length(Varia_paysage_multi), style = 3)
  rep=0
  for (i in colnames(Varia_paysage_multi)) {
    ...
    for (j in colnames(Varia_paysage_multi)) {
      ...
      # if(shapiro.test(chloe_cor[,1])$p > 0.05 & shapiro.test(chloe_cor[,1])$p > 0.05)
      temp=cor.test(Varia_paysage_multi[,i],
                    Varia_paysage_multi[,j],
                    method = method_cor)
